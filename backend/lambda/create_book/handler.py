...
def handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    try:
        ...
        log_request("create_book", event, user_id)
        payload = json.loads(event.get("body") or "{}")
        isbn = normalize_isbn(str(payload.get("isbn", "")))

        if not isbn:
            return log_response("create_book", json_response(400, {"message": "Invalid ISBN"}))
        ...
        if not category_id:
            return log_response("create_book", json_response(400, {"message": "Invalid categoryId"}))

        if book_format not in BOOK_FORMATS:
            return log_response("create_book", json_response(400, {"message": "Invalid bookFormat"}))
        ...
        if reading_status not in READING_STATUSES:
            return log_response("create_book", json_response(400, {"message": "Invalid readingStatus"}))
        ...
        return log_response(
            "create_book",
            json_response(201, to_book_response(item, category_name=category["name"])),
        )
    except ClientError as error:
        if error.response["Error"]["Code"] == "ConditionalCheckFailedException":
            return log_response("create_book", json_response(409, {"message": "Book already exists"}))
        return log_response("create_book", json_response(500, {"message": f"Failed to create book: {error}"}))
