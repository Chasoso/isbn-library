...
def handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    try:
        ...
        log_request("get_book", event, user_id)
        raw_isbn = (event.get("pathParameters") or {}).get("isbn", "")
        isbn = normalize_isbn(raw_isbn)

        if not isbn:
            return log_response("get_book", json_response(400, {"message": "Invalid ISBN"}))
        ...
        if not item:
            return log_response("get_book", json_response(404, {"message": "Book not found"}))
        ...
        return log_response("get_book", json_response(500, {"message": f"Failed to get book: {error}"}))
