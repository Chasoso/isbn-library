...
def handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    try:
        ...
        log_request("update_category", event, user_id)
        category_id = ((event.get("pathParameters") or {}).get("categoryId") or "").strip()
        payload = json.loads(event.get("body") or "{}")

        if not category_id:
            return log_response("update_category", json_response(400, {"message": "Invalid categoryId"}))
        ...
        if name is not None:
            ...
            if not trimmed_name:
                return log_response("update_category", json_response(400, {"message": "Category name is required"}))
            ...
            if any(
                ...
                return log_response("update_category", json_response(409, {"message": "Category already exists"}))
        ...
        if "sortOrder" in payload:
            try:
                ...
                return log_response("update_category", json_response(400, {"message": "Invalid sortOrder"}))
            updates.append("sortOrder = :sortOrder")
            values[":sortOrder"] = sort_order

        if not updates:
            return log_response("update_category", json_response(400, {"message": "No fields to update"}))
        ...
        return log_response(
            "update_category",
            json_response(200, category_response(result.get("Attributes", {}))),
        )
    except ClientError as error:
        if error.response["Error"]["Code"] == "ConditionalCheckFailedException":
            return log_response("update_category", json_response(404, {"message": "Category not found"}))
        return log_response("update_category", json_response(500, {"message": f"Failed to update category: {error}"}))
